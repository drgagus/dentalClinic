      ...
      <div class="col-lg-12 p-0">
        ...
        <div class="tab-content border border-info bg-light" id="myTabContent">
          <div class="tab-pane fade show px-2 py-2 active" id="home" role="tabpanel" aria-labelledby="home-tab">
              ...
              <a href="/apotek/pasien/{{$dentalrecord->id}}#home" class="text-primary btn btn-sm btn-outline-primary">back</a>
                <div class="row">
                    <div class="col-lg-6">
                        <form action="/apotek/pasien/{{$dentalrecord->id}}/edit" class="" method="post">
                        @csrf
                        @method('patch')
                            <table class="table table-hover border border-dark mt-1" style="width:100%">
                                    ...
                                    <input type="hidden" class="" name="medicinerecord_id{{$medicinerecord->id}}" value={{$medicinerecord->id}}>
                                    <tr class="">
                                        <td class="py-1" style="width:10%">#</td>
                                        <td class="py-1" style="width:80%">
                                            <select class="form-control" name="obat{{$medicinerecord->id}}" readonly>
                                                <option value="{{$medicinerecord->medicine_id}}">{{$medicinerecord->medicine->obat}}</option>
                                            </select>
                                        </td>
                                        <td class="py-1" style="width:10%">
                                            <input type="text" class="form-control" name="jumlah{{$medicinerecord->id}}" value="{{old('jumlah'.$medicinerecord->id) ?? $medicinerecord->jumlah}}">
                                        </td>
                                    </tr>
                                    @endforeach
                                    <tr class="">
